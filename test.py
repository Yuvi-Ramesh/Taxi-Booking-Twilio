...
TWILIO_NUMBER = "+12495015490"
MY_PHONE_NUMBER = "+918148480180"
...
def get_distance_duration_from_google(pickup, drop):
    """
    Use Google Distance Matrix API to get driving distance (km) & duration (minutes).
    """
    try:
        url = (
            "https://maps.googleapis.com/maps/api/distancematrix/json"
            f"?origins={urllib.parse.quote_plus(pickup)}"
            f"&destinations={urllib.parse.quote_plus(drop)}"
            f"&mode=driving&units=metric&key={GOOGLE_API_KEY}"
        )
        resp = requests.get(url).json()
        print("🌍 Google API response:", resp)
...
def calculate_trip_cost(pickup, drop, vehicle_type, pickup_date=None, pickup_time=None):
    """
    Calculate trip cost using Google Distance Matrix (preferred).
    If Google fails, fallback to Gemini estimation.
    """
    ...
    distance_km, duration_min = get_distance_duration_from_google(pickup, drop)

    if not distance_km:  # fallback to Gemini
